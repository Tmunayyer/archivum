...
The design is settled and the **tracer bullet is in** (issue #5): `archivum run --source <dir> --dest <dir> --scheme <name>` labels a folder end-to-end with a hand-seeded scheme of `label` fields — no preview yet. The batch interactions landed with #6: per-field recents (top three, arrow keys, `-1`-cursor free-form state), shift+tab back-a-field, and in-file progress. Capture-date ordering landed with #7: `capturedate` (seam 2) resolves the whole batch in one exiftool pass — tag priority, `-api QuickTimeUTC=1` per ADR-0010, tested against real exiftool and committed fixture media. All three field types landed with #9: a scheme slot is a typed `run.Field` (store `fieldKeys` declare the type; an undeclared key stays `label`), `number` enforces its grammar per keystroke and skips normalization (ADR-0009's field-type boundaries section), and `date` rides the offer list — capture date, then today — with `YYYY-MM-DD` enforced on confirm. Inline previews landed with #8: `internal/preview` (lifted from the prototype, no delete escape anywhere) renders images directly and videos as a three-frame strip, the run model emits each file's payload via `tea.Println` through an optional `Deps.Previewer` (a render failure prints its reason and the file is still prompted), and preflight now checks exiftool/ffmpeg/ffprobe/chafa and refuses `$TMUX`. Inline scheme creation landed with #10, closing out #4: a run against an unknown scheme name opens with an offer to compose it (`run.NewComposing`, a composition phase ahead of the file loop on the same model), field keys are named one at a time with every stored key offered for reuse tagged with its type (ADR-0003's namespace defence), a new key is asked its type exactly once, key names normalize like values, esc finishes, and the scheme is saved — via the same Deps-as-tea.Cmd side-effect route — before the first file is prompted, with its keys and types printed into scrollback. Packages live under `internal/` (`normalize`, `compose`, `source`, `capturedate`, `preview`, `store`, `run`, `cmd`); `run` is seam 1, driven in tests by `tea.KeyMsg` sequences against fakes (plus a real store for recency and composition behaviour).
