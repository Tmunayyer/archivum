...
The design is settled and the **tracer bullet is in** (issue #5): `archivum run --source <dir> --dest <dir> --scheme <name>` labels a folder end-to-end with a hand-seeded scheme of `label` fields — no preview yet. The batch interactions landed with #6: per-field recents (top three, arrow keys, `-1`-cursor free-form state), shift+tab back-a-field, and in-file progress. Capture-date ordering landed with #7: `capturedate` (seam 2) resolves the whole batch in one exiftool pass — tag priority, `-api QuickTimeUTC=1` per ADR-0010, tested against real exiftool and committed fixture media — and the run model's batch is `[]run.File` carrying each file's capture date for #9's date field. Packages live under `internal/` (`normalize`, `compose`, `source`, `capturedate`, `store`, `run`, `cmd`); `run` is seam 1, driven in tests by `tea.KeyMsg` sequences against fakes (plus a real store for recency behaviour). Previews, field types beyond `label`, and inline scheme creation are later tickets under #4.
