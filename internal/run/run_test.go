...
// batch wraps bare source paths as run.Files; capture dates stay zero —
// nothing at seam 1 reads them until the date field type lands (#9).
func batch(paths ...string) []run.File {
...
func TestComposition(t *testing.T) {
	t.Run("values join in scheme order, extension preserved, one copy and one save per file", func(t *testing.T) {
		...
		dest := &fakeDest{}
		m := run.New(
			batch("/src/IMG_0001.MOV", "/src/IMG_0002.jpg"),
...
func TestCollisions(t *testing.T) {
	t.Run("existing names gain _n incrementing past taken suffixes", func(t *testing.T) {
		...
		m := run.New(batch("/src/a.mp4"), []string{"movement"}, run.Deps{Store: &fakeStore{}, Dest: dest})
...
func TestNormalization(t *testing.T) {
	t.Run("typed values are normalized in both the composed name and the store", func(t *testing.T) {
		st := &fakeStore{}
		dest := &fakeDest{}
		m := run.New(batch("/src/a.jpg"), []string{"movement", "place"}, run.Deps{Store: st, Dest: dest})
...
func TestEmptyRejection(t *testing.T) {
	t.Run("an empty answer and one normalizing to nothing are both refused", func(t *testing.T) {
		dest := &fakeDest{}
		m := run.New(batch("/src/a.jpg"), []string{"movement"}, run.Deps{Store: &fakeStore{}, Dest: dest})
...
func TestCopySemantics(t *testing.T) {
	t.Run("files are copied once each through the real dest and sources are untouched", func(t *testing.T) {
		...
		m := run.New(batch(srcs...), []string{"movement"}, run.Deps{Store: &fakeStore{}, Dest: run.DirDest{Dir: destDir}})
...
func TestRecents(t *testing.T) {
	...
	t.Run("a field with history lists the top three most-recent-first", func(t *testing.T) {
		m := run.New(batch("/src/a.mp4"), []string{"movement"}, run.Deps{Store: seed(t), Dest: &fakeDest{}})
		view := m.View()
	...
	t.Run("enter alone confirms the top recent", func(t *testing.T) {
		dest := &fakeDest{}
		m := run.New(batch("/src/a.mp4"), []string{"movement"}, run.Deps{Store: seed(t), Dest: dest})
		drive(t, m, enter)
	...
	t.Run("a field with no history shows no list", func(t *testing.T) {
		m := run.New(batch("/src/a.mp4"), []string{"movement"}, run.Deps{Store: &fakeStore{}, Dest: &fakeDest{}})
	...
	t.Run("a typed value tops the list on the very next file", func(t *testing.T) {
		...
		m := run.New(batch("/src/a.jpg", "/src/b.jpg"), []string{"movement"}, run.Deps{Store: st, Dest: &fakeDest{}})
	...
	t.Run("recents are global per field key, shared across schemes", func(t *testing.T) {
		st := testStore(t)
		a := run.New(batch("/src/a.jpg"), []string{"movement", "weight-lb"}, run.Deps{Store: st, Dest: &fakeDest{}})
		drive(t, a, flatten(answer("squat"), answer("185"))...)

		b := run.New(batch("/src/b.jpg"), []string{"movement"}, run.Deps{Store: st, Dest: &fakeDest{}})
...
func TestBackAField(t *testing.T) {
	t.Run("shift+tab pre-fills the previous answer editable with the cursor at the end", func(t *testing.T) {
		dest := &fakeDest{}
		m := run.New(batch("/src/a.jpg"), []string{"movement", "weight-lb"}, run.Deps{Store: &fakeStore{}, Dest: dest})
		drive(t, m, flatten(
			answer("bench"),
	...
	t.Run("re-advancing after going back keeps the later answers pre-filled", func(t *testing.T) {
		dest := &fakeDest{}
		m := run.New(batch("/src/a.jpg"), []string{"movement", "weight-lb", "date"}, run.Deps{Store: &fakeStore{}, Dest: dest})
		drive(t, m, flatten(
	...
	t.Run("enter after going back confirms the pre-filled value, not a recent", func(t *testing.T) {
		...
		dest := &fakeDest{}
		m := run.New(batch("/src/a.jpg"), []string{"movement", "weight-lb"}, run.Deps{Store: st, Dest: dest})
		drive(t, m, flatten(
			answer("bench"),
...
func TestCopyFailure(t *testing.T) {
	t.Run("a copy failure stops the run naming the file it was on", func(t *testing.T) {
		...
		m := run.New(batch("/src/IMG_0042.jpg"), []string{"movement"}, run.Deps{Store: st, Dest: dest})
		m = drive(t, m, flatten(answer("squat"))...)
